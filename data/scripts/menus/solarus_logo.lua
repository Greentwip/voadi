...
-- Thanks to fontenddev.com for help with the 8-bit graphic!
...
-- White background
local background = sol.surface.create(256, 144)
background:fill_color{255, 255, 255}

-- Main surface of the menu.
local logo_surface = sol.surface.create(144, 35)
...
local white_square = sol.surface.create(32, 16)
white_square:fill_color{255, 255, 255}
...
local function rebuild_surface()
  ...
  logo_surface:fill_color{255, 255, 255}

  -- Draw the title (after step 1).
  if animation_step >= 1 then
    title:draw(logo_surface, 0, 1)
  end

  -- Draw the sun.
  sun:draw(logo_surface, 18, 32)

  -- Draw the white square to partially hide the sun.
  white_square:draw(logo_surface, 18, 24)

  -- Draw the sword.
  sword:draw(logo_surface, 51, -27)

  -- Draw the subtitle (after step 2).
  if animation_step >= 2 then
    subtitle:draw(logo_surface, 0, 27)
  end
end
...
function solarus_logo_menu:on_draw(screen)
  ...
  logo_surface:draw(background, width / 2 - 72, height / 2 - 26)
end
